    ...
    <table class="table table-bordered table-stripped js_panel_images-row">
        ...
        @foreach($model->images()->ofType($type)->get() as $image)
            <tr class="js_panel_images-col">
                ...
                <td>
                    <textarea name="{{ $name }}{{ $nameAfter }}[description][{{ $imageKey }}]" rows="5" class="form-control" placeholder="Описание" data-name-before="{{ $name }}" data-name-after="{{ $nameAfter }}[description][{{ $imageKey }}]">{{ $image->description }}</textarea>
                </td>
                <td>
        ...
        <tr class="js_panel_images-col js_panel_images-col-clone">
            <td>
                <div class="js_panel_images-wrapper">
                    ...
                    <input class="js_panel_images-order" type="hidden" name="{{ $name }}{{ $nameAfter }}[order][]" value="" data-name-before="{{ $name }}" data-name-after="{{ $nameAfter }}[order][]"/>
